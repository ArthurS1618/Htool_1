...
class HMatrix : public VirtualHMatrix<T> {
  ...
  // Je rajoute pour pouvoir avoir le blocktree
  
  Block<T>* get_BlockTree(){
    auto res =this->BlockTree.get();
    return res;}
  std::vector<Block<T>*> get_ComputedBlock(){return this->MyComputedBlocks;}
