...
// };
  ...
  /////////////////////////////
class MyMatrix : public VirtualGenerator<complex<double>> {
    const vector<double> &p1;
    const vector<double> &p2;
    int space_dim;
...
int main(int argc, char *argv[]) {
  ...
  vector<double> p = Nodes;
    // Hmatrix
MyMatrix A(3, size, size,p,p,"/work/sauniear/Documents/matrice_Xavier/matrice_arthur.txt" );
 std::vector<complex<double>> x(size, 1);
 std::vector<complex<double>> result(size, 0);
    ...
    HMatrix<complex<double>> HA(t, t, epsilon, eta, 'N', 'N');
    HA.build(A, p.data());
    result = HA * x;
    ...
    std::cout << Frobenius_absolute_error(HA, A) / A.norme() << std::endl;
