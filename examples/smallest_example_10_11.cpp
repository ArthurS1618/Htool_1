...
// Constructeur avec tout
   ...
   //getters
	...
	vectro<Matrix<double> sres; vector<Block<double>> shres; vector<int> ofres;
	int oft = t.get_offset();  int szt = t.get_size(); 
	int ofs = s.get_offset() ; int szs = s.get_size();
	int nr = sr.size()/2 ; int nh = sh.size()/2;
	//On commence par prendre les restrictions des low rank
	for (int n = 0 ; n < nr ; ++n){
		Matrix<double> U= sr[2*n]; Matrix<double> V = sr[2*n+1];
		int ofu = of[2*n] ; int ofv = of[2*n+1];
		...
		if ( (ofu >= oft) and ( szt+(oft-ofu) <= U.nb_rows() )  and ( ofv >= ofs ) and ( szs+(ofs-ofv))){
			...
			sres.push_back(Uk); sres.push_bacj(Vk); ofres.push_back(oft); ofres.push_back(ofs) ; }
		}
	...
	for (int n =0; n < nh; ++n){
		...
		if ( (H.get_target_cluster().get_size() == szt) and ( H.get_target_cluster().get_offset() == oft) and ( K.get_source_cluster().get_size() ==szs) and (K.get_source_cluster().get_offset() == ofs ) ){
			...
			for (int k =0; k< H.nb_sons(); ++k){
				...
				int szh = Hk.get_target_cluster().get_size(); int ofh = Hk.get_target_cluster().get_size();}
